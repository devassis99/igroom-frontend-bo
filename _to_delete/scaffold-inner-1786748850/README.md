...
All nine sidebar sections from the mockup (B1–B8, with Billing & Plans since split into two:
...
future modal is needed) creates just the product shell, "Add Price" adds a billing-cycle price to
an existing one, and "Create Payment Link" builds a shareable checkout link scoped to a product,
one or more billing cycles, and optional seat/trial overrides, a discount, and an expiry. The
mockup doesn't show what triggers Add Price, so it's wired to a "+ Price" action underneath each
row's product name — the table's own "LINK" column was needed for the per-price "Copy" action the
mockup does show. "Copy" (per row) and "Copy Link" (in the modal) both do a real
`navigator.clipboard.writeText`, since that's genuine client-side behavior with no backend
required; the link text itself (`pay.igroom.io/c/<product-slug>?...`) is illustrative — igroom
backend has no Stripe integration yet, and the modal's fields don't submit anywhere ("Generate
Link" just closes the modal, matching "Create Product"/"Create Price" elsewhere on this page).
**Billing** (`BillingPage.tsx`) kept the renewals/failed-payments half of what used to be the
combined page.

One inconsistency in the B4z (Billing) mockup frame worth flagging: it draws a narrower,
differently-colored sidebar with no user/logout footer, unlike every other frame. Treated that as
a mockup authoring artifact rather than an intentional per-page redesign — Billing uses the same
`AppShell` as everything else here, so the app doesn't have two different sidebar styles.
