...
product, billing cycle, status) plus two modals: "New Product" (`Modal.tsx`, reused wherever a
future modal is needed) creates just the product shell, and "Add Price" adds a billing-cycle price
to an existing one. The mockup doesn't show what triggers Add Price, so it's wired to a "+ Price"
action on each row. **Billing** (`BillingPage.tsx`) kept the renewals/failed-payments half of what
used to be the combined page. igroom-backend has no Stripe integration yet, so both "Create
Product" and "Create Price" just close their modal locally.
