...
`DashboardPage.tsx` (Overview) reproduces the mockup's stat cards + top-shops table using the
mockup's own illustrative numbers, clearly labeled as sample data — igroom-backend has no
metrics/reporting endpoint yet. The other seven sidebar sections (Usage & Notifications,
Shops/Accounts, Billing & Plans, Bookings & Waitlist, Support Tickets, Referrals & Rewards,
Platform Settings) are wired up as real routes so the nav is honest, but render a "not built out
yet" placeholder (`ComingSoonPage.tsx`) until there's a backend endpoint to back each one.

## Still to do

- Pull the actual design from `claude.ai/design` (pending `/design consent`) and reskin
  `src/components/layout/AppShell.tsx` + the page components — everything currently styles off
  the placeholder tokens in `src/styles/index.css`.
- Role-aware navigation/route guards once there's more than one screen — `bo_roles`/`bo_permissions`
  exist on the backend already but nothing here reads them yet.
- Real screens beyond the dashboard placeholder.
