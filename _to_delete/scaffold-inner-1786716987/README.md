...
Billing & Plans additionally has a **Stripe Products** table and a "New Stripe Product" modal
(`Modal.tsx`, reused wherever a future modal is needed), synced from the mockup's B4/B4b revision —
igroom-backend has no Stripe integration yet, so "Create in Stripe" just closes the modal locally.
