# Deploying the back office to S3
...
```
push to main → pnpm lint, test → fetch igroom/frontend-bo → .env.production
...
```bash
export REGION=eu-west-1 BUCKET=igroom-app-bo
...
CloudFront in front sooner rather than later, along with `admin.igroom.app`.

## The secret

One secret per app, holding only what the browser needs: `VITE_API_BASE_URL`, `VITE_GOOGLE_CLIENT_ID`.

```bash
aws secretsmanager create-secret --name igroom/frontend-bo --secret-string '{
...
- `secretsmanager:GetSecretValue` on the `igroom/frontend-bo` ARN
- `s3:PutObject`, `s3:ListBucket` on `arn:aws:s3:::igroom-app-bo` and `/*`
